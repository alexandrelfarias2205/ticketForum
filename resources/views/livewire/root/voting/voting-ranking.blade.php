<div>
    {{-- Cabeçalho --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Ranking de Votação</h1>
        <p class="mt-1 text-sm text-gray-500">Sugestões ordenadas pelo número de votos recebidos.</p>
    </div>

    {{-- Filtros --}}
    <div class="mb-4 flex flex-wrap gap-3">
        <input
            type="text"
            wire:model.live="search"
            placeholder="Buscar por título..."
            class="rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 w-64"
        />

        <select
            wire:model.live="filterType"
            class="rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
        >
            <option value="">Todos os tipos</option>
            <option value="bug">Bug</option>
            <option value="improvement">Melhoria</option>
            <option value="feature_request">Nova Funcionalidade</option>
        </select>

        <select
            wire:model.live="filterTenant"
            class="rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
        >
            <option value="">Todas as empresas</option>
            @foreach($this->tenants as $tenant)
                <option value="{{ $tenant->id }}">{{ $tenant->name }}</option>
            @endforeach
        </select>
    </div>

    {{-- Tabela --}}
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        @if($this->reports->isNotEmpty())
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600 w-14">#</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600">Título</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600">Empresa</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600">Tipo</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600">Status</th>
                        <th class="px-4 py-3 text-center font-semibold text-gray-600">Votos</th>
                        <th class="px-4 py-3 text-right font-semibold text-gray-600">Ações</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach($this->reports as $index => $report)
                        @php
                            $position = ($this->reports->currentPage() - 1) * $this->reports->perPage() + $index + 1;
                            $borderClass = match(true) {
                                $position === 1 => 'border-l-4 border-l-yellow-400',
                                $position === 2 => 'border-l-4 border-l-gray-400',
                                $position === 3 => 'border-l-4 border-l-amber-600',
                                default => '',
                            };
                        @endphp
                        <tr class="hover:bg-gray-50 {{ $borderClass }}">
                            <td class="px-4 py-3 font-bold text-gray-500">
                                @if($position === 1)
                                    <span class="text-yellow-500">1</span>
                                @elseif($position === 2)
                                    <span class="text-gray-400">2</span>
                                @elseif($position === 3)
                                    <span class="text-amber-600">3</span>
                                @else
                                    {{ $position }}
                                @endif
                            </td>
                            <td class="px-4 py-3 font-medium text-gray-900 max-w-xs">
                                <span class="truncate block">{{ $report->title }}</span>
                                @if($report->labels->isNotEmpty())
                                    <div class="mt-1 flex flex-wrap gap-1">
                                        @foreach($report->labels as $label)
                                            <span class="inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium"
                                                  style="background-color: {{ $label->color }}20; color: {{ $label->color }};">
                                                {{ $label->name }}
                                            </span>
                                        @endforeach
                                    </div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-600">{{ $report->tenant?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $report->type->label() }}</td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $report->status->badgeClasses() }}">
                                    {{ $report->status->label() }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="text-lg font-bold text-indigo-700">{{ $report->vote_count }}</span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                @if($report->status === \App\Enums\ReportStatus::PublishedForVoting)
                                    <a href="{{ route('root.reports.show', $report) }}"
                                       class="text-indigo-600 hover:text-indigo-800 font-medium text-xs">
                                        Ver revisão
                                    </a>
                                @else
                                    <span class="text-gray-400 text-xs">—</span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>

            @if($this->reports->hasPages())
                <div class="border-t border-gray-100 px-4 py-3">
                    {{ $this->reports->links() }}
                </div>
            @endif
        @else
            <div class="flex flex-col items-center justify-center py-16 text-center">
                <svg class="h-12 w-12 text-gray-300 mb-3" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                </svg>
                <p class="text-gray-500 font-medium">Nenhuma sugestão em votação no momento.</p>
            </div>
        @endif
    </div>
</div>
