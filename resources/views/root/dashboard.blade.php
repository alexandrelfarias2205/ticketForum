<x-layouts.root title="Painel">
    @php
        $tenantsTotal    = \App\Models\Tenant::count();
        $tenantsActive   = \App\Models\Tenant::where('is_active', true)->count();
        $usersTotal      = \App\Models\User::count();
        $reportsTotal    = \App\Models\Report::withoutGlobalScope(\App\Models\Scopes\TenantScope::class)->count();
        $reportsPending  = \App\Models\Report::withoutGlobalScope(\App\Models\Scopes\TenantScope::class)->where('status', \App\Enums\ReportStatus::PendingReview->value)->count();
        $reportsPublished   = \App\Models\Report::withoutGlobalScope(\App\Models\Scopes\TenantScope::class)->where('status', \App\Enums\ReportStatus::PublishedForVoting->value)->count();
        $reportsInProgress  = \App\Models\Report::withoutGlobalScope(\App\Models\Scopes\TenantScope::class)->where('status', \App\Enums\ReportStatus::InProgress->value)->count();
        $reportsDone        = \App\Models\Report::withoutGlobalScope(\App\Models\Scopes\TenantScope::class)->where('status', \App\Enums\ReportStatus::Done->value)->count();
    @endphp
    ...
    <div class="space-y-6">
        ...
        <section aria-label="Indicadores" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <x-stat-card label="Empresas" :value="$tenantsTotal" tone="brand" :hint="$tenantsActive . ' ativas'" :href="route('root.tenants.index')">
                <x-slot name="icon">
            ...
            <x-stat-card label="Usuários" :value="$usersTotal" tone="info" hint="Em todos os tenants" :href="route('root.users.index')">
                <x-slot name="icon">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
            ...
            <x-stat-card label="Aguardando revisão" :value="$reportsPending" tone="warning" hint="Tickets para triagem" :href="route('root.reports.index')">
                <x-slot name="icon">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
            ...
            <x-stat-card label="Total de tickets" :value="$reportsTotal" tone="accent" hint="Todos os status">
                <x-slot name="icon">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
